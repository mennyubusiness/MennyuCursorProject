...
function joinCodeButtonClassName() {
  return "rounded-lg border border-oo-light-stone bg-oo-warm-white px-3 py-1.5 text-xs font-semibold text-oo-charcoal hover:bg-oo-cream";
}
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {browsePodId && hasServerSession ? (
              ...
              <Link
                ...
                onClick={onNavigate}
                className={joinCodeButtonClassName()}
              >
                Start group order
              </Link>
            ) : null}
            <button type="button" onClick={openJoinModal} className={joinCodeButtonClassName()}>
              Join with code
            </button>
          </div>
        </section>
