  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-oo-light-stone bg-oo-warm-white p-4">
        <h2 className="font-medium text-oo-charcoal">Summary</h2>
        <dl className="mt-2 grid gap-1 text-sm">
          <div>
            <span className="text-oo-stone-gray">Created:</span> {formatDate(adminOrder.createdAt)}
          </div>
          <div>
            <span className="text-oo-stone-gray">Status:</span>{" "}
          ...
          <div>
            <span className="text-oo-stone-gray">Customer phone:</span> {adminOrder.customerPhone}
          </div>
          {adminOrder.customerEmail && (
            <div>
              <span className="text-oo-stone-gray">Customer email:</span> {adminOrder.customerEmail}
            </div>
          )}
          <div>
            <span className="text-oo-stone-gray">Pod:</span> {adminOrder.pod.name}
          </div>
          {adminOrder.orderNotes && (
            <div>
              <span className="text-oo-stone-gray">Checkout notes:</span> {adminOrder.orderNotes}
            </div>
          )}
          <div>
            <span className="text-oo-stone-gray">Subtotal:</span> ${(adminOrder.subtotalCents / 100).toFixed(2)}
          </div>
          <div>
            <span className="text-oo-stone-gray">Total:</span> ${(adminOrder.totalCents / 100).toFixed(2)}
          </div>
        </dl>

        <h3 className="mt-4 text-xs font-semibold uppercase tracking-wide text-oo-stone-gray">Vendors</h3>
        <ul className="mt-2 divide-y divide-oo-light-stone">
          {vendorContexts.map(({ vo, deliverectBadges, exceptionType, showRecoveredBadge }) => (
            <li key={vo.id} className="flex flex-wrap items-center gap-x-2 gap-y-1 py-2 first:pt-0">
              ...
              <span className="text-sm text-oo-stone-gray">{fulfillmentLabel(vo.fulfillmentStatus)}</span>
              ...
              <span className="text-xs text-oo-stone-gray">${(vo.totalCents / 100).toFixed(2)}</span>
            </li>
          ))}
        </ul>
      ...
      {vendorContexts.some((c) => c.showActionsPanel) && (
        <section className="rounded-lg border border-oo-light-stone bg-oo-warm-white p-4">
          <h2 className="text-lg font-semibold text-oo-charcoal">Primary actions</h2>
          ...
          <div className="mt-4 space-y-4">
            {vendorContexts
              ...
              .map(({ vo, exceptionType, actionState, reason, showProgressionUi, progressionTargetsFiltered }) => (
                <div key={vo.id} className="rounded-lg border border-oo-light-stone bg-oo-cream/50 p-4">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    <h3 className="font-semibold text-oo-charcoal">{vo.vendor.name}</h3>
                    <Link
      ...
      <section className="rounded-lg border border-oo-light-stone bg-oo-warm-white p-4">
        <h2 className="text-lg font-semibold text-oo-charcoal">Vendor orders</h2>
        <p className="mt-1 text-sm text-oo-stone-gray">Line items and refund status per vendor slice.</p>
        <div className="mt-4 space-y-6">
          {vendorContexts.map(({ vo }) => {
            ...
            return (
              <div key={vo.id} className="border-t border-oo-light-stone pt-4 first:border-t-0 first:pt-0">
                ...
                {vo.fulfillmentStatus === "cancelled" && (() => {
                  if (latestRefund?.status === "succeeded") {
                    return (
                      <p className="mt-2 rounded border border-oo-light-stone bg-oo-cream/80 p-2 text-xs font-medium text-oo-charcoal">
                ...
                <ul className="mt-3 space-y-2">
                  {vo.lineItems.map((line) => (
                    <li key={line.id} className="border-b border-oo-light-stone pb-2 text-sm last:border-0">
      ...
      {vendorContexts.some((c) => isDeliverectRecheckEligible(c.vo)) && (
        <section className="rounded-lg border border-oo-light-stone bg-oo-warm-white p-4">
          <h2 className="text-lg font-semibold text-oo-charcoal">Deliverect reconciliation</h2>
          <p className="mt-1 text-sm text-oo-stone-gray">
          ...
          <div className="mt-4 space-y-4">
            {vendorContexts
              ...
              .map(({ vo }) => (
                <div key={vo.id} className="rounded-lg border border-oo-light-stone bg-oo-cream/50 p-3">
                  <p className="text-sm font-medium text-oo-charcoal">{vo.vendor.name}</p>
      ...
      {timeline.length > 0 && (
        <details className="rounded-lg border border-oo-light-stone bg-oo-warm-white p-4">
          <summary className="cursor-pointer text-lg font-semibold text-oo-charcoal">
            Order timeline
          </summary>
          <p className="mt-1 text-sm text-oo-stone-gray">
          ...
          <ul className="mt-4 space-y-3">
            {timeline.map((e) => (
              ...
              >
                <span className="shrink-0 text-xs text-oo-stone-gray">{formatDate(e.at)}</span>
      ...
      {/* 6. Debug (collapsed) */}
      <details className="rounded-lg border border-oo-light-stone bg-oo-cream/80 p-4">
        <summary className="cursor-pointer text-sm font-semibold text-oo-charcoal">Technical details</summary>
        <div className="mt-3 space-y-3 font-mono text-xs text-oo-charcoal">
          <p>
            <span className="text-oo-stone-gray">Order ID:</span> {adminOrder.id}
          </p>
          {vendorContexts.map(({ vo }) => (
            <p key={vo.id}>
              <span className="text-oo-stone-gray">Vendor order {vo.vendor.name}:</span> {vo.id} · routing{" "}
