...
describe("admin audit log helpers", () => {
  ...
  it("defines sprint action types", () => {
    ...
    expect(ADMIN_AUDIT_ACTION.PAYMENT_VALIDATION_RERUN).toBe("PAYMENT_VALIDATION_RERUN");
    expect(ADMIN_AUDIT_ACTION.TRANSACTIONAL_SMS_RESENT).toBe("TRANSACTIONAL_SMS_RESENT");
  });
});
